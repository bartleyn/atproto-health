...
async function scanPds(pdsUrl: string): Promise<ScanResult> {
  ...
  try {
    ...
    if (counts.total > 0) {
      throw Object.assign(err as Error, { partialCounts: counts });
    }
    throw err;
  }
}
...
async function main() {
  ...
  const upsert = db.prepare(`
    INSERT INTO pds_repo_status_snapshots
      (pds_url, snapshot_date, active, deactivated, deleted, takendown, suspended, other, total_scanned, is_sampled)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0)
  ...
  await runWithConcurrency(toScan, async (pdsUrl, _idx) => {
    ...
    try {
      const result = await scanPds(pdsUrl);
